...
class Svg {
  ...
  constructor (params) {
    ...
    this.chartSlices = params.chartSlices
    this.labels = params.labels
    this.isTeam = params.isTeam
    this.element = this.buildCompleteSvg(this.chartId, this.chartSlices, this.labels, this.isTeam)
  }

  processSlice (groupData, groupIndex) {
    const slice = d3.select(this.svg.selectAll('.slice').nodes()[groupIndex])
    const designerId = groupData[0].designerId
    const category = groupData[0].category
    const label = groupData[0].label

    create.paths(slice, groupData, this.isTeam, this.chartSlices, this.labels)
    const scoreData = calculate.score(groupData)
    const totalSlices = calculate.totalSlices(this.isTeam, this.chartSlices, this.labels)
  ...
  buildCompleteSvg (chartId, chartSlices, labels, isTeam = false) {
